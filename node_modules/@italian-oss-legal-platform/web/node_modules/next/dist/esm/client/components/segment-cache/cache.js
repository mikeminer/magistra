...
export async function fetchSegmentsOnCacheMiss(route, routeKey, tree, segments, segmentCount) {
    // This function is allowed to use async/await because it contains the actual
    // fetch that gets issued on a cache miss. Notice it writes the result to the
    // cache entry directly, rather than return data that is then written by
    // the caller.
    ...
    try {
        ...
        while(node !== null && dataIndex < serverDataArray.length){
            ...
            const canonicalVaryPath = process.env.__NEXT_VARY_PARAMS && data.varyParams !== null ? getFulfilledSegmentVaryPath(node.tree.varyPath, data.varyParams) : getSegmentVaryPathForRequest(FetchStrategy.PPR, node.tree);
            let fulfilled = null;
            ...
            if (nodeEntry !== null && nodeEntry.status === 1) {
                ...
                fulfilled = fulfillSegmentCacheEntry(upgradeToPendingSegment(detachedEntry, FetchStrategy.PPR), data.rsc, entryStaleAt, data.isPartial);
            }
