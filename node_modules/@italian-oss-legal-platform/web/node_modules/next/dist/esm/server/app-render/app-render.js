...
import { chainStreams, continueFizzStream, continueDynamicPrerender, continueStaticPrerender, continueDynamicHTMLResumeNode, continueDynamicHTMLResumeWeb, continueStaticFallbackPrerender, streamToBuffer, streamToString, createWebInlinedDataStream, createPendingStream, createOnHeadersCallback, resumeAndAbort, renderToWebFlightStream, resumeToFizzStream, getServerPrerender, getClientPrerender, processPrelude as processPreludeOp, createDocumentClosingStream, teeStream, renderToWebFizzStream, renderToNodeFlightStream, renderToNodeFizzStream, createNodeInlinedDataStream } from './stream-ops';
...
import { createReactServerPrerenderResult, ReactServerResult, ReplayableNodeStream, createReactServerPrerenderResultFromRender } from './app-render-prerender-utils';
...
import { createWebDebugChannel, createNodeDebugChannel } from './debug-channel-server';
 ...
 */ async function generateDynamicFlightRenderResult(req, ctx, requestStore, options) {
    ...
    if (process.env.__NEXT_USE_NODE_STREAMS) {
        ...
        const flightStream = workUnitAsyncStorage.run(requestStore, renderToNodeFlightStream, ctx.componentMod, rscPayload, clientModules, {
            onError,
        ...
        const { clientModules } = getClientReferenceManifest();
        const rscPayload = await workUnitAsyncStorage.run(requestStore, generateDynamicRSCPayload, ctx, options);
        const flightStream = workUnitAsyncStorage.run(requestStore, renderToWebFlightStream, ctx.componentMod, rscPayload, clientModules, {
            onError,
            temporaryReferences: options == null ? void 0 : options.temporaryReferences,
            filterStackFrame,
            debugChannel: debugChannel == null ? void 0 : debugChannel.serverSide
 ...
 * content.
 */ async function generateStagedDynamicFlightRenderResultWeb(req, ctx, requestStore) {
 ...
 */ async function generateStagedDynamicFlightRenderResultNode(req, ctx, requestStore) {
    ...
    const flightStream = await runInSequentialTasks(()=>{
        stageController.advanceStage(RenderStage.Static);
        const sourceStream = workUnitAsyncStorage.run(requestStore, renderToNodeFlightStream, ctx.componentMod, rscPayload, clientModules, {
            onError,
            filterStackFrame
        });
        ...
        countStaticStageBytesNode(staticStream, stageController).then(resolveStaticStageByteLength);
        return dynamicStream;
    }, ()=>{
        // This is a separate task that doesn't advance a stage. It forces
        // draining the microtask queue so that the stale time iterable and vary
...
async function stagedRenderWithoutCachesInDevWeb(ctx, requestStore, getPayload, options) {
    ...
    return await runInSequentialTasks(()=>{
        stageController.advanceStage(RenderStage.Static);
        return workUnitAsyncStorage.run(requestStore, renderToWebFlightStream, ctx.componentMod, rscPayload, clientModules, {
            ...options,
            environmentName
        });
    }, ()=>{
...
async function stagedRenderWithoutCachesInDevNode(ctx, requestStore, getPayload, options) {
    ...
    return await runInSequentialTasks(()=>{
        stageController.advanceStage(RenderStage.Static);
        return workUnitAsyncStorage.run(requestStore, renderToNodeFlightStream, ctx.componentMod, rscPayload, clientModules, {
            ...options,
            environmentName
        });
    }, ()=>{
 ...
 */ async function generateDynamicFlightRenderResultWithStagesInDev(req, ctx, initialRequestStore, createRequestStore, fallbackParams) {
    ...
    !isBypassingCachesInDev(initialRequestStore)) {
        ...
        const { stream: serverStream, accumulatedChunksPromise, syncInterruptReason, startTime, staticStageEndTime, runtimeStageEndTime, debugChannel: returnedDebugChannel, requestStore: finalRequestStore } = await (process.env.__NEXT_USE_NODE_STREAMS ? renderWithRestartOnCacheMissInDevNode(ctx, initialRequestStore, createRequestStore, getPayload, onError) : renderWithRestartOnCacheMissInDevWeb(ctx, initialRequestStore, createRequestStore, getPayload, onError));
        if (shouldValidate) {
        ...
        if (process.env.__NEXT_USE_NODE_STREAMS) {
            ...
            stream = await stagedRenderWithoutCachesInDevWeb(ctx, initialRequestStore, getPayload, {
                onError: onError,
                filterStackFrame,
                debugChannel: debugChannel == null ? void 0 : debugChannel.serverSide
            });
...
async function getRSCPayload(tree, ctx, options) {
    ...
    const initialTree = await createFlightRouterStateFromLoaderTree(tree, hints, prefetchInliningEnabled, ctx.renderOpts.cacheComponents, workStore.isStaticGeneration, ctx.renderOpts.isBuildTimePrerendering ?? false, getDynamicParamFromSegment, query);
    const serveStreamingMetadata = !!ctx.renderOpts.serveStreamingMetadata;
...
async function getErrorRSCPayload(tree, ctx, ssrError, errorType) {
    ...
    const initialTree = await createFlightRouterStateFromLoaderTree(tree, errorHints, errorPrefetchInliningEnabled, ctx.renderOpts.cacheComponents, workStore.isStaticGeneration, ctx.renderOpts.isBuildTimePrerendering ?? false, getDynamicParamFromSegment, query);
    let err = undefined;
    if (ssrError) {
...
async function renderToHTMLOrFlightImpl(req, res, url, pagePath, query, renderOpts, workStore, parsedRequestHeaders, postponedState, serverComponentsHmrCache, sharedContext, interpolatedParams, fallbackRouteParams) {
    ...
    if (ComponentMod.__next_app__) {
        ...
        const isTracingEnabled = ((_getTracer_getActiveScopeSpan = getTracer().getActiveScopeSpan()) == null ? void 0 : _getTracer_getActiveScopeSpan.isRecording()) ?? false;
            ...
            if (isRuntimePrefetchRequest) {
                ...
                if (process.env.__NEXT_DEV_SERVER && process.env.NEXT_RUNTIME !== 'edge' && cacheComponents) {
                    ...
                    if (process.env.__NEXT_USE_NODE_STREAMS) {
                        ...
                        return generateStagedDynamicFlightRenderResultWeb(req, ctx, requestStore);
                    }
                } else {
                    // MARK: RSC dynamic
            ...
            cacheComponents) {
                if (process.env.__NEXT_USE_NODE_STREAMS) {
                    ...
                    !isBypassingCachesInDev(requestStore)) {
                        const { stream: serverStream, accumulatedChunksPromise, syncInterruptReason, startTime, staticStageEndTime, runtimeStageEndTime, debugChannel: returnedDebugChannel, requestStore: finalRequestStore } = await renderWithRestartOnCacheMissInDevNode(ctx, requestStore, createRequestStore, getPayload, serverComponentsErrorHandler);
                        let validationDebugChannelClient = undefined;
                        if (returnedDebugChannel) {
                            const [t1, t2] = teeStream(returnedDebugChannel.clientSide.readable);
                            returnedDebugChannel.clientSide.readable = t1;
                    ...
                    if (debugChannel && setReactDebugChannel) {
                        const [readableSsr, readableBrowser] = teeStream(debugChannel.clientSide.readable);
                    ...
                    // "disable cache" in devtools or a hard refresh (cache-control: "no-store")
                    !isBypassingCachesInDev(requestStore)) {
                        const { stream: serverStream, accumulatedChunksPromise, syncInterruptReason, startTime, staticStageEndTime, runtimeStageEndTime, debugChannel: returnedDebugChannel, requestStore: finalRequestStore } = await renderWithRestartOnCacheMissInDevWeb(ctx, requestStore, createRequestStore, getPayload, serverComponentsErrorHandler);
                        let validationDebugChannelClient = undefined;
                        if (returnedDebugChannel) {
                            const [t1, t2] = teeStream(returnedDebugChannel.clientSide.readable);
                            returnedDebugChannel.clientSide.readable = t1;
                        ...
                        // Do a dynamic render, but with (basic) environment labels.
                        debugChannel = setReactDebugChannel && createWebDebugChannel();
                        const serverStream = await stagedRenderWithoutCachesInDevWeb(ctx, requestStore, getPayload, {
                            onError: serverComponentsErrorHandler,
                            filterStackFrame,
                            debugChannel: debugChannel == null ? void 0 : debugChannel.serverSide
                        });
                ...
                if (process.env.__NEXT_USE_NODE_STREAMS) {
                    ...
                    const flightStream = await runInSequentialTasks(()=>{
                        stageController.advanceStage(RenderStage.Static);
                        const stream = workUnitAsyncStorage.run(requestStore, renderToNodeFlightStream, ctx.componentMod, RSCPayload, clientModules, {
                            onError: serverComponentsErrorHandler,
                            filterStackFrame
                        });
                ...
                if (process.env.__NEXT_USE_NODE_STREAMS) {
                    ...
                    reactServerResult = new ReactServerResult(workUnitAsyncStorage.run(requestStore, renderToNodeFlightStream, ctx.componentMod, RSCPayload, clientModules, {
                        filterStackFrame,
                        onError: serverComponentsErrorHandler,
                        debugChannel: debugChannel == null ? void 0 : debugChannel.serverSide
                    }));
                    ...
                    reactServerResult = new ReactServerResult(workUnitAsyncStorage.run(requestStore, renderToWebFlightStream, ctx.componentMod, RSCPayload, clientModules, {
                        filterStackFrame,
                        onError: serverComponentsErrorHandler,
                        debugChannel: debugChannel == null ? void 0 : debugChannel.serverSide
                    }));
            ...
            if (process.env.__NEXT_USE_NODE_STREAMS) {
                ...
                if (typeof renderOpts.postponed === 'string') {
                    if ((postponedState == null ? void 0 : postponedState.type) === DynamicState.DATA) {
                        ...
                        const inlinedDataStream = createNodeInlinedDataStream(reactServerResult.tee(), nonce, formState);
                        ...
                        return await continueDynamicHTMLResumeNode(htmlStream, {
                            ...
                            inlinedDataStream: createNodeInlinedDataStream(reactServerResult.consume(), nonce, formState),
                ...
                const { stream: htmlStream, allReady } = await workUnitAsyncStorage.run(requestStore, renderToNodeFizzStream, appElement, fizzOptions);
                // End the render span only after React completed rendering (including anything inside Suspense boundaries)
                allReady.finally(()=>{
                    if (renderSpan.isRecording()) renderSpan.end();
                });
                ...
                if (typeof renderOpts.postponed === 'string') {
                    if ((postponedState == null ? void 0 : postponedState.type) === DynamicState.DATA) {
                        ...
                        // still include the new server component render because it was not included
                        // in the static prelude.
                        const inlinedDataStream = createWebInlinedDataStream(reactServerResult.tee(), nonce, formState);
                        // End the span since there's no async rendering in this path
                        if (renderSpan.isRecording()) renderSpan.end();
                        return chainStreams(inlinedDataStream, createDocumentClosingStream());
                    } else if (postponedState) {
                        ...
                        return await continueDynamicHTMLResumeWeb(htmlStream, {
                            delayDataUntilFirstHtmlChunk: preludeState === DynamicHTMLPreludeState.Empty,
                            inlinedDataStream: createWebInlinedDataStream(reactServerResult.consume(), nonce, formState),
                            getServerInsertedHTML,
                            getServerInsertedMetadata,
                            deploymentId: ctx.sharedContext.deploymentId
                        });
                ...
                const { stream: htmlStream, allReady } = await workUnitAsyncStorage.run(requestStore, renderToWebFizzStream, appElement, fizzOptions);
                // End the render span only after React completed rendering (including anything inside Suspense boundaries)
                allReady.finally(()=>{
                    if (renderSpan.isRecording()) renderSpan.end();
                });
        ...
        // MARK: renderToStream errorRecovery
            ...
            if (process.env.__NEXT_USE_NODE_STREAMS) {
                ...
                try {
                    ...
                    errorServerStream = workUnitAsyncStorage.run(requestStore, renderToNodeFlightStream, ctx.componentMod, errorRSCPayload, clientModules, {
                        filterStackFrame,
                        onError: serverComponentsErrorHandler
                    });
                ...
                try {
                    const generateStaticHTML = supportsDynamicResponse !== true || !!shouldWaitOnAllReady;
                    const { stream: errorHtmlStream, allReady: errorAllReady } = await workUnitAsyncStorage.run(requestStore, renderToNodeFizzStream, /*#__PURE__*/ _jsx(ErrorApp, {
                    ...
                    return await continueFizzStream(errorHtmlStream, {
                        inlinedDataStream: createNodeInlinedDataStream(// This is intentionally using the readable datastream from the
                ...
                try {
                    errorRSCPayload = await workUnitAsyncStorage.run(requestStore, getErrorRSCPayload, tree, ctx, reactServerErrorsByDigest.has(err.digest) ? null : err, errorType);
                    errorServerStream = workUnitAsyncStorage.run(requestStore, renderToWebFlightStream, ctx.componentMod, errorRSCPayload, clientModules, {
                        filterStackFrame,
                        onError: serverComponentsErrorHandler
                    });
                    if (reactServerResult === null) {
                ...
                try {
                    const generateStaticHTML = supportsDynamicResponse !== true || !!shouldWaitOnAllReady;
                    const { stream: errorHtmlStream, allReady: errorAllReady } = await workUnitAsyncStorage.run(requestStore, renderToWebFizzStream, /*#__PURE__*/ _jsx(ErrorApp, {
                        reactServerStream: errorServerStream,
                        ServerInsertedHTMLProvider: ServerInsertedHTMLProvider,
                        preinitScripts: errorPreinitScripts,
                        nonce: nonce,
                    ...
                    return await continueFizzStream(errorHtmlStream, {
                        inlinedDataStream: createWebInlinedDataStream(// This is intentionally using the readable datastream from the
                        // main render rather than the flight data from the error page
                        // render
                        reactServerResult.consume(), nonce, formState),
                        isStaticGeneration: generateStaticHTML,
...
async function renderWithRestartOnCacheMissInDevWeb(ctx, initialRequestStore, createRequestStore, getPayload, onError) {
    ...
    const initialStreamResult = await runInSequentialTasks(()=>{
        ...
        const streamPair = teeStream(workUnitAsyncStorage.run(requestStore, renderToWebFlightStream, ComponentMod, initialRscPayload, clientModules, {
            onError,
            environmentName,
            startTime,
            filterStackFrame,
    ...
    const finalStreamResult = await runInSequentialTasks(()=>{
        finalStageController.advanceStage(RenderStage.EarlyStatic);
        startTime = performance.now() + performance.timeOrigin;
        const streamPair = teeStream(workUnitAsyncStorage.run(requestStore, renderToWebFlightStream, ComponentMod, finalRscPayload, clientModules, {
            onError,
            environmentName,
            startTime,
            filterStackFrame,
...
async function renderWithRestartOnCacheMissInDevNode(ctx, initialRequestStore, createRequestStore, getPayload, onError) {
    const { htmlRequestId, renderOpts } = ctx;
    const { ComponentMod, setCacheStatus, setReactDebugChannel } = renderOpts;
    let startTime = -Infinity;
    // If the render is restarted, we'll recreate a fresh request store
    ...
    const initialStreamResult = await runInSequentialTasks(()=>{
        initialStageController.advanceStage(RenderStage.EarlyStatic);
        startTime = performance.now() + performance.timeOrigin;
        const sourceStream = workUnitAsyncStorage.run(requestStore, renderToNodeFlightStream, ComponentMod, initialRscPayload, clientModules, {
            onError,
            environmentName,
            startTime,
            filterStackFrame,
        ...
        const accumulatedChunksPromise = accumulateStreamChunks(replayable.createReplayStream(), initialStageController, initialDataController.signal);
        initialDataController.signal.addEventListener('abort', ()=>{
            accumulatedChunksPromise.catch(()=>{});
            stream.destroy();
        }, {
    ...
    const finalStreamResult = await runInSequentialTasks(()=>{
        finalStageController.advanceStage(RenderStage.EarlyStatic);
        startTime = performance.now() + performance.timeOrigin;
        const finalSourceStream = workUnitAsyncStorage.run(requestStore, renderToNodeFlightStream, ComponentMod, finalRscPayload, clientModules, {
            onError,
            environmentName,
            startTime,
            filterStackFrame,
        ...
        return {
            ...
            accumulatedChunksPromise: accumulateStreamChunks(finalReplayable.createReplayStream(), finalStageController, null)
        };
    }, ()=>{
        // Static stage
        finalStageController.advanceStage(RenderStage.Static);
    ...
    if (stream instanceof ReadableStream) {
        ...
        try {
            while(!cancelled){
                ...
                accumulateChunk(stageController, staticChunks, runtimeChunks, dynamicChunks, value);
            }
        } catch (err) {
...
function accumulateChunk(stageController, staticChunks, runtimeChunks, dynamicChunks, value) {
...
async function countStaticStageBytesNode(stream, stageController) {
    let byteLength = 0;
    let cancelled = false;
    stageController.onStage(RenderStage.EarlyRuntime, ()=>{
        cancelled = true;
 ...
 */ async function logMessagesAndSendErrorsToBrowser(messages, ctx) {
    ...
    if (errors.length > 0) {
        ...
        if (process.env.__NEXT_USE_NODE_STREAMS) {
            errorsFlightStream = renderToNodeFlightStream(ctx.componentMod, {
                errors,
                errorCodes
            }, clientModules, {
                filterStackFrame
            ...
            errorsFlightStream = renderToWebFlightStream(ctx.componentMod, {
                errors,
                errorCodes
            }, clientModules, {
                filterStackFrame
 ...
 */ async function validateInstantConfigs(accumulatedChunks, debugChunks, startTime, rootParams, fallbackRouteParams, ctx, hmrRefreshHash, validationSamples) {
    ...
    const renderFlightStream = process.env.__NEXT_USE_NODE_STREAMS ? renderToNodeFlightStream : renderToWebFlightStream;
    const createDebugChannel = process.env.__NEXT_USE_NODE_STREAMS ? createNodeDebugChannel : createWebDebugChannel;
    const { cache, payload: initialRscPayload, stageEndTimes } = await collectStagedSegmentData(ctx.componentMod, renderFlightStream, {
    ...
    }, debugChunks, startTime, hasRuntimePrefetch, clientReferenceManifest, createDebugChannel);
    const { implicitTags, nonce, workStore } = ctx;
   ...
   */ async function validateAtDepth(depth, groupDepthForValidation) {
    ...
    async function validateAtDepthImpl(depth, groupDepthForValidation, previousBoundaryState) {
        ...
        const preinitScripts = ()=>{};
        const { ServerInsertedHTMLProvider } = createServerInsertedHTML();
        const { stream: serverStream, debugStream } = await createCombinedPayloadStream(ctx.componentMod, renderFlightStream, payloadResult.payload, extraChunksController, reactController.signal, clientReferenceManifest, startTime, isDebugChannelEnabled, createDebugChannel);
        const instantValidationState = createInstantValidationState(payloadResult.slotStacks);
 ...
 */ async function renderWithRestartOnCacheMissInValidation(ctx, initialRequestStore, createRequestStore, getPayload, createOnError, prefilledDataCache) {
    ...
    const initialResult = await runInSequentialTasks(()=>{
        initialStageController.advanceStage(RenderStage.EarlyStatic);
        startTime = performance.now() + performance.timeOrigin;
        const stream = workUnitAsyncStorage.run(requestStore, renderFlightStream, ComponentMod, initialRscPayload, clientModules, {
    ...
    const finalResult = await runInSequentialTasks(()=>{
        finalStageController.advanceStage(RenderStage.EarlyStatic);
        startTime = performance.now() + performance.timeOrigin;
        const stream = workUnitAsyncStorage.run(requestStore, renderFlightStream, ComponentMod, finalRscPayload, clientModules, {
...
async function validateInstantConfigsInBuild(ctx, prefilledDataCache) {
    const run = async ()=>{
        ...
        try {
            ...
            console.error(Object.defineProperty(new InvariantError('An unexpected error occurred during instant validation', {
                cause: err
            }), "__NEXT_ERROR_CODE", {
                value: "E1097",
                enumerable: false,
...
async function prerenderToStream(req, res, ctx, metadata, tree, fallbackRouteParams) {
    ...
    try {
       ...
       */ // The prerender controller represents the lifetime of the prerender. It
            ...
            if (serverIsDynamic) {
                ...
                if (postponed != null) {
                    ...
                    const resumePrelude = await workUnitAsyncStorage.run(finalServerPrerenderStore, resumeAndAbort, // eslint-disable-next-line @next/internal/no-ambiguous-jsx
                    /*#__PURE__*/ _jsx(App, {
                ...
                if (hasFallbackRouteParams) {
                    ...
                    const emptyReactServerResult = await createReactServerPrerenderResultFromRender(renderToWebFlightStream(ComponentMod, [], clientModules, {
                        filterStackFrame,
                        onError: serverComponentsErrorHandler
                    }));
                    finalStream = await continueStaticFallbackPrerender(htmlStream, {
                        inlinedDataStream: createWebInlinedDataStream(emptyReactServerResult.consumeAsStream(), nonce, formState),
                        getServerInsertedHTML,
                        getServerInsertedMetadata,
                        deploymentId: ctx.sharedContext.deploymentId
                    });
                    ...
                    finalStream = await continueStaticPrerender(htmlStream, {
                        inlinedDataStream: createWebInlinedDataStream(reactServerResult.consumeAsStream(), nonce, formState),
                        getServerInsertedHTML,
                        getServerInsertedMetadata,
                        deploymentId: ctx.sharedContext.deploymentId
                    });
            ...
            reactServerResult = reactServerPrerenderResult = await createReactServerPrerenderResultFromRender(workUnitAsyncStorage.run(reactServerPrerenderStore, renderToWebFlightStream, ComponentMod, RSCPayload, clientModules, {
                filterStackFrame,
                onError: serverComponentsErrorHandler
            }));
       ...
       */ // First we check if we have any dynamic holes in our HTML prerender
            ...
            let reactServerResult;
            reactServerResult = reactServerPrerenderResult = await createReactServerPrerenderResultFromRender(workUnitAsyncStorage.run(prerenderLegacyStore, renderToWebFlightStream, ComponentMod, RSCPayload, clientModules, {
                filterStackFrame,
                onError: serverComponentsErrorHandler
            }));
            const { stream: htmlStream } = await workUnitAsyncStorage.run(prerenderLegacyStore, renderToWebFizzStream, // eslint-disable-next-line @next/internal/no-ambiguous-jsx
            /*#__PURE__*/ _jsx(App, {
                reactServerStream: reactServerResult.asUnclosingStream(),
                reactDebugStream: undefined,
                debugEndTime: undefined,
        ...
        if (cacheComponents && isHTTPAccessFallbackError(err)) {
            const triggeredStatus = getAccessFallbackHTTPStatus(err);
        ...
        const errorRSCPayload = prerenderHTTPError ? await workUnitAsyncStorage.run(prerenderLegacyStore, getRSCPayload, tree, ctx, {
            is404: errorType === 'not-found',
            prerenderHTTPError
        }) : await workUnitAsyncStorage.run(prerenderLegacyStore, getErrorRSCPayload, tree, ctx, reactServerErrorsByDigest.has(err.digest) ? undefined : err, errorType);
        const errorServerStreamRaw = workUnitAsyncStorage.run(prerenderLegacyStore, renderToWebFlightStream, ComponentMod, errorRSCPayload, clientModules, {
            filterStackFrame,
            onError: serverComponentsErrorHandler
        });
        ...
        try {
            const { stream: errorHtmlStream } = await workUnitAsyncStorage.run(prerenderLegacyStore, renderToWebFizzStream, // eslint-disable-next-line @next/internal/no-ambiguous-jsx
            ...
            return {
                ...
                stream: await continueFizzStream(errorHtmlStream, {
                    inlinedDataStream: createWebInlinedDataStream(flightStream, nonce, formState),
                    isStaticGeneration: true,
                    getServerInsertedHTML: makeGetServerInsertedHTML({
                        polyfills,
                        renderServerInsertedHTML,
