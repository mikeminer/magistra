...
import { getOverlayMiddleware, getSourceMapMiddleware, getOriginalStackFrames } from './middleware-webpack';
import { WebpackHotMiddleware } from './hot-middleware';
import * as inspector from 'inspector';
import { join, relative, isAbsolute, posix, dirname } from 'path';
import { createEntrypoints, finalizeEntrypoint, getClientEntry, getEdgeServerEntry, getAppEntry, runDependingOnPageType, getInstrumentationEntry } from '../../build/entries';
...
import { BLOCKED_PAGES, CLIENT_STATIC_FILES_RUNTIME_MAIN, CLIENT_STATIC_FILES_RUNTIME_MAIN_APP, CLIENT_STATIC_FILES_RUNTIME_REACT_REFRESH, COMPILER_NAMES, RSC_MODULE_TYPES } from '../../shared/lib/constants';
...
export default class HotReloaderWebpack {
    ...
    onHMR(req, _socket, head, callback) {
        wsServer.handleUpgrade(req, req.socket, head, (client)=>{
            ...
            if (!this.webpackHotMiddleware) {
                throw Object.defineProperty(new InvariantError('Did not start HotReloaderWebpack.'), "__NEXT_ERROR_CODE", {
                    value: "E804",
                    enumerable: false,
                    configurable: true
                });
                        ...
                        case 'browser-logs':
                            {
                                await receiveBrowserLogsWebpack({
                                    ...
                                    config: this.config.logging && this.config.logging.browserToTerminal || false
                                });
                                break;
                            }
                        case 'ping':
            ...
            if (htmlRequestId) {
                ...
                sendSerializedErrorsToClientForHtmlRequest(htmlRequestId, this.sendToClient.bind(this, client));
    ...
    async clean(span) {
        return span.traceChild('clean').traceAsyncFn(()=>recursiveDeleteSyncWithAsyncRetries(join(this.dir, this.config.distDir), /^(cache|lock)/));
    }
    ...
    async start() {
        ...
        await Promise.all(Array.from(getCacheDirectories(this.activeWebpackConfigs)).map(checkFileSystemCacheInvalidationAndCleanup));
        ...
        this.webpackHotMiddleware = new WebpackHotMiddleware(this.multiCompiler.compilers, this.versionInfo, this.devtoolsFrontendUrl, this.config, initialDevToolsConfig);
        let booted = false;
        ...
        this.middlewares.push(getOverlayMiddleware({
            rootDirectory: this.dir,
            ...
            serverStats: ()=>this.serverStats,
            edgeServerStats: ()=>this.edgeServerStats
        }), getNextErrorFeedbackMiddleware(this.telemetry), getDevOverlayFontMiddleware(), getDisableDevIndicatorMiddleware(), getRestartDevServerMiddleware({
        ...
        }), getAttachNodejsDebuggerMiddleware(), ...this.config.experimental.mcpServer ? [
            getMcpMiddleware({
