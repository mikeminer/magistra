/* eslint-disable import/no-extraneous-dependencies */ import { renderToReadableStream, decodeReply, decodeReplyFromAsyncIterable, createTemporaryReferenceSet as createServerTemporaryReferenceSet } from 'react-server-dom-webpack/server';
...
import { getHmrRefreshHash, getRenderResumeDataCache, getPrerenderResumeDataCache, workUnitAsyncStorage, getDraftModeProviderForCacheScope, getCacheSignal, isHmrRefresh, getServerComponentsHmrCache } from '../app-render/work-unit-async-storage.external';
import { applyOwnerStack, getRuntimeStage, makeDevtoolsIOAwarePromise, makeHangingPromise } from '../dynamic-rendering-utils';
import { getClientReferenceManifest, getServerModuleMap } from '../app-render/manifests-singleton';
 ...
 */ class ResolvableSharedCacheResult {
    ...
    constructor(){
        this.deferred = createPromiseWithResolvers();
        this.registrations = [];
    }
}
/**
 ...
 */ function saveSharedCacheEntryToResumeDataCache(serializedCacheKey, sharedCacheEntry, prerenderResumeDataCache) {
    if (!prerenderResumeDataCache || prerenderResumeDataCache.cache.has(serializedCacheKey)) {
        return;
    }
...
function saveToCacheHandler(cacheHandler, workStore, id, serializedCacheKey, savedCacheResult, rootParams) {
    const pendingCoarseEntry = savedCacheResult.then((collectedResult)=>{
        ...
        if (rootParamNames && rootParamNames.size > 0 && rootParams) {
            ...
            const rootParamTags = [
                ...rootParamNames
            ].map((paramName)=>NEXT_CACHE_ROOT_PARAM_TAG_ID + paramName);
            return {
...
function generateCacheEntryWithRestoredWorkStore(workStore, cacheContext, clientReferenceManifest, encodedArguments, fn, timeoutError) {
    ...
    return workAsyncStorage.run(workStore, generateCacheEntryWithCacheContext, workStore, cacheContext, clientReferenceManifest, encodedArguments, fn, timeoutError);
}
...
async function generateCacheEntryImpl(workStore, cacheContext, innerCacheStore, clientReferenceManifest, encodedArguments, fn, timeoutError) {
    ...
    const handleError = createReactServerErrorHandler(process.env.NODE_ENV === 'development', workStore.isBuildTimePrerendering ?? false, workStore.reactServerErrorsByDigest, (error)=>{
    ...
    switch(outerWorkUnitStore.type){
        ...
        case 'prerender':
            ...
            if (timeoutAbortController.signal.aborted) {
                ...
                const hangingPromise = makeHangingPromise(outerWorkUnitStore.renderSignal, workStore.route, 'dynamic "use cache"');
        ...
        case 'request':
            ...
            if (process.env.__NEXT_DEV_SERVER && outerWorkUnitStore.cacheSignal) {
                ...
                if ((stagedRendering == null ? void 0 : stagedRendering.currentStage) !== RenderStage.Dynamic) {
...
export async function cache(kind, id, boundArgsLength, originalFn, args) {
    ...
    if (workUnitStore === undefined) {
        throw Object.defineProperty(new InvariantError('"use cache" cannot be used outside of App Router. Expected a WorkUnitStore.'), "__NEXT_ERROR_CODE", {
            value: "E1135",
            enumerable: false,
            configurable: true
        });
            ...
            case 'request':
                {
                    if (process.env.NODE_ENV === 'development') {
                        ...
                        const stagedRendering = outerWorkUnitStore.stagedRendering;
                        const stage = stagedRendering ? getRuntimeStage(stagedRendering) : RenderStage.Runtime;
    ...
    let cacheHandlerKey = knownRootParamNames && rootParams ? serializedCacheKey + computeRootParamsCacheKeySuffix(rootParams, knownRootParamNames) : serializedCacheKey;
    ...
    if (renderResumeDataCache) {
        var _renderResumeDataCache_dynamicCacheKeys;
        ...
        if ((_renderResumeDataCache_dynamicCacheKeys = renderResumeDataCache.dynamicCacheKeys) == null ? void 0 : _renderResumeDataCache_dynamicCacheKeys.has(serializedCacheKey)) {
            switch(workUnitStore.type){
                case 'prerender':
                case 'prerender-runtime':
                    return makeHangingPromise(workUnitStore.renderSignal, workStore.route, 'dynamic "use cache"');
        ...
        if (rdcEntry !== undefined) {
            ...
            if (rdcResult !== undefined) {
                if (rdcResult.entry.revalidate === 0 || rdcResult.entry.expire < DYNAMIC_EXPIRE) {
                    switch(workUnitStore.type){
                        case 'prerender':
                            ...
                            if (rdcResult.entry.revalidate === 0) {
                                if (rdcResult.hasExplicitRevalidate === false) {
                                    throw wrapAsInvalidDynamicUsageError(Object.defineProperty(new Error(nestedCacheZeroRevalidateErrorMessage), "__NEXT_ERROR_CODE", {
                                        value: "E1000",
                                        enumerable: false,
                                        configurable: true
                                    }), workStore);
                                ...
                                if (rdcResult.hasExplicitExpire === false) {
                                    throw wrapAsInvalidDynamicUsageError(Object.defineProperty(new Error(nestedCacheShortExpireErrorMessage), "__NEXT_ERROR_CODE", {
                                        value: "E1009",
                                        enumerable: false,
                                        configurable: true
                                    }), workStore);
                        ...
                        case 'request':
                            {
                                if (process.env.NODE_ENV === 'development') {
                                    if (rdcResult.entry.revalidate === 0 && rdcResult.hasExplicitRevalidate === false) {
                                        throw wrapAsInvalidDynamicUsageError(Object.defineProperty(new Error(nestedCacheZeroRevalidateErrorMessage), "__NEXT_ERROR_CODE", {
                                            value: "E1000",
                                            enumerable: false,
                                            configurable: true
                                    ...
                                    if (rdcResult.entry.expire < DYNAMIC_EXPIRE && rdcResult.hasExplicitExpire === false) {
                                        throw wrapAsInvalidDynamicUsageError(Object.defineProperty(new Error(nestedCacheShortExpireErrorMessage), "__NEXT_ERROR_CODE", {
                                            value: "E1009",
                                            enumerable: false,
                                            configurable: true
                ...
                if (rdcResult.entry.stale < RUNTIME_PREFETCH_DYNAMIC_STALE) {
                    switch(workUnitStore.type){
                        case 'prerender-runtime':
                            ...
                            debug == null ? void 0 : debug('omitting entry', serializedCacheKey, 'from runtime shell due to short stale value:', rdcResult.entry.stale);
                            if (cacheSignal) {
                                cacheSignal.endRead();
                            }
                            return makeHangingPromise(workUnitStore.renderSignal, workStore.route, 'dynamic "use cache"');
                        case 'request':
                            {
                                if (process.env.NODE_ENV === 'development') {
                                    ...
                                    // and thus will cause a restart even if all caches are filled.
                                    await makeDevtoolsIOAwarePromise(undefined, workUnitStore, RenderStage.Dynamic);
                                }
                                break;
                            }
                        case 'prerender':
    ...
    if (stream === undefined) {
        ...
        if (intraRequestPendingCacheInvocation) {
            ...
            saveSharedCacheEntryToResumeDataCache(serializedCacheKey, sharedCacheResult.entry, prerenderResumeDataCache);
    ...
    if (stream === undefined) {
        ...
        try {
            ...
            while(stream === undefined){
                ...
                const crossRequestPendingCacheInvocation = isPrivate ? undefined : crossRequestPendingCacheInvocations.get(cacheHandlerKey);
                if (crossRequestPendingCacheInvocation) {
                    ...
                    if (sharedCacheResult.type === 'cached') {
                        ...
                        if (updatedRootParamNames && rootParams) {
                            const newCacheHandlerKey = serializedCacheKey + computeRootParamsCacheKeySuffix(rootParams, updatedRootParamNames);
                ...
                if (cacheHandler && !shouldForceRevalidate(workStore, workUnitStore)) {
                    ...
                    if (entry && rootParams) {
                        ...
                        if (paramNames.size > 0) {
                            ...
                            cacheHandlerKey = serializedCacheKey + computeRootParamsCacheKeySuffix(rootParams, paramNames);
                            entry = await cacheHandler.get(cacheHandlerKey, implicitTags);
                        }
                    }
                }
                ...
                if (entry !== undefined && (entry.revalidate === 0 || entry.expire < DYNAMIC_EXPIRE)) {
                    switch(workUnitStore.type){
                        case 'prerender':
                            ...
                            const hangingPromise = makeHangingPromise(workUnitStore.renderSignal, workStore.route, 'dynamic "use cache"');
                ...
                if (entry === undefined || currentTime > entry.timestamp + entry.expire * 1000 || workStore.isStaticGeneration && currentTime > entry.timestamp + entry.revalidate * 1000) {
                    ...
                    if (!workStore.isDraftMode) {
                        const savedCacheResult = saveToResumeDataCache(prerenderResumeDataCache, serializedCacheKey, pendingCacheResult);
                        if (cacheHandler) {
                            saveToCacheHandler(cacheHandler, workStore, id, serializedCacheKey, savedCacheResult, rootParams);
                        }
                    }
                    ...
                    if (prerenderResumeDataCache) {
                        ...
                        prerenderResumeDataCache.cache.set(serializedCacheKey, Promise.resolve({
                            entry: entryRight,
                    ...
                    if (currentTime > entry.timestamp + entry.revalidate * 1000) {
                        ...
                        // and tags back to the outer scope.
                        {
                            ...
                            outerOwnerStack: cacheContext.outerOwnerStack
                        }, clientReferenceManifest, encodedCacheKeyParts, fn, timeoutError).then(async (result)=>{
                            if (result.type === 'cached') {
