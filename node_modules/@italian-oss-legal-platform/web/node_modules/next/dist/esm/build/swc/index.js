...
export var HmrTarget = /*#__PURE__*/ function(HmrTarget) {
    HmrTarget["Client"] = "client";
    HmrTarget["Server"] = "server";
    return HmrTarget;
}({});
const nextVersion = "16.3.0-canary.6";
const ArchName = arch();
...
const triples = (()=>{
    ...
    if (rawTargetTriple) {
        Log.warn(`next-swc does not have native bindings support for target triple ${rawTargetTriple}. Native features like Turbopack will not be available.`);
    } else {
        Log.warn(`next-swc does not have native bindings for platform ${PlatformName}/${ArchName}. Native features like Turbopack will not be available.`);
    }
    return [];
})();
...
const normalizePathOnWindows = (p)=>path.sep === '\\' ? p.replace(/\\/g, '/') : p;
...
function bindingToApi(binding, bindingPath, _wasm) {
   ...
   */ function subscribe(useBuffer, nativeFunction) {
    ...
    async function rustifyProjectOptions(options) {
        return {
            ...options,
            nextConfig: await serializeNextConfig(options.nextConfig, path.join(options.rootPath, options.projectPath)),
    ...
    async function rustifyPartialProjectOptions(options) {
        return {
            ...options,
            nextConfig: options.nextConfig && await serializeNextConfig(options.nextConfig, path.join(options.rootPath, options.projectPath)),
    ...
    async function serializeNextConfig(nextConfig, projectPath) {
        ...
        if (nextConfigSerializable.images.loaderFile) {
            nextConfigSerializable.images = {
                ...nextConfigSerializable.images,
                loaderFile: './' + normalizePathOnWindows(path.relative(projectPath, nextConfigSerializable.images.loaderFile))
            };
        }
        // cacheHandler can be an absolute path, we need it to be relative for turbopack.
        if (nextConfigSerializable.cacheHandler) {
            const resolvedCacheHandler = resolveCacheHandlerPathToFilesystem(nextConfigSerializable.cacheHandler);
            nextConfigSerializable.cacheHandler = './' + normalizePathOnWindows(path.isAbsolute(resolvedCacheHandler) ? path.relative(projectPath, resolvedCacheHandler) : resolvedCacheHandler);
        }
        if (nextConfigSerializable.cacheHandlers) {
            nextConfigSerializable.cacheHandlers = Object.fromEntries(Object.entries(nextConfigSerializable.cacheHandlers).filter(([_, value])=>value != null).map(([key, value])=>{
                ...
                return [
                    key,
                    './' + normalizePathOnWindows(path.isAbsolute(resolved) ? path.relative(projectPath, resolved) : resolved)
                ];
            }));
        }
