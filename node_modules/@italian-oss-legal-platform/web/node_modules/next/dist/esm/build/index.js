    ...
    return new Worker(staticWorkerPath, {
        ...
        forkOptions: {
            env: {
                ...
                __NEXT_PRERENDER_CLIENT_ASSET_SUFFIX: config.experimental.supportsImmutableAssets || !config.deploymentId ? '' : `?dpl=${config.deploymentId}`
            }
        }
    });
}
    ...
    try {
        ...
        await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            if (config.cleanDistDir && !isGenerateMode) {
                await nextBuildSpan.traceChild('clean').traceAsyncFn(()=>recursiveDeleteSyncWithAsyncRetries(distDir, /^(cache|dev|lock|trace)/));
            }
            ...
            await nextBuildSpan.traceChild('generate-route-types').traceAsyncFn(async ()=>{
                ...
                const cacheLifeFilePath = path.join(distDir, 'types', 'cache-life.d.ts');
                ...
                await writeRootParamsTypes(routeTypesManifest, path.join(distDir, 'types', 'root-params.d.ts'), config);
            });
            ...
            await recordFrameworkVersion("16.3.0-canary.6");
            ...
            const requiredServerFilesManifest = await nextBuildSpan.traceChild('generate-required-server-files').traceAsyncFn(async ()=>{
                ...
                for (const [key, value] of Object.entries(runtimeConfig.cacheHandlers || {})){
                    if (key && value) {
                        normalizedCacheHandlers[key] = path.relative(distDir, resolveCacheHandlerPathToFilesystem(value));
                    }
                }
                ...
                const serverFilesManifest = {
                    ...
                    config: {
                        ...
                        cacheHandler: runtimeConfig.cacheHandler ? path.relative(distDir, resolveCacheHandlerPathToFilesystem(runtimeConfig.cacheHandler)) : runtimeConfig.cacheHandler,
            ...
            const { customAppGetInitialProps, namedExports, isNextImageImported, hasNonStaticErrorPage } = await staticCheckSpan.traceAsyncFn(async ()=>{
                ...
                const errorPageStaticResult = nonStaticErrorPageSpan.traceAsyncFn(async ()=>{
                    ...
                    return hasCustomErrorPage && staticWorker.isPageStatic({
                        ...
                        buildId,
                        clientAssetToken: config.experimental.supportsImmutableAssets ? '' : config.deploymentId,
                        sriEnabled,
