...
export async function getTypeScriptConfiguration(typescript, tsConfigPath, metaOnly) {
    try {
        ...
        if (isError(err) && err.name === 'SyntaxError') {
            const reason = '\n' + (err.message ?? '');
            throw Object.defineProperty(new Error(bold('Could not parse' + cyan('tsconfig.json') + '.' + ' Please make sure it contains syntactically correct JSON.') + reason), "__NEXT_ERROR_CODE", {
                value: "E1023",
                enumerable: false,
                configurable: true
            });
