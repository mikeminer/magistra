...
class AppRouteRouteModule extends _routemodule.RouteModule {
   ...
   */ async ensureUserland() {
    ...
    constructor({ userland, getUserland, definition, distDir, relativeProjectDir, resolvedPagePath, nextConfigOutput }){
        ...
        super({
            userland: isLazy ? undefined : userland,
            definition,
            distDir,
            relativeProjectDir
        }), /**
   ...
   * cookies.
   */ this.actionAsyncStorage = _actionasyncstorageexternal.actionAsyncStorage, // Non-null while an async userland module (top-level await) is loading.
    ...
    _initFromUserland() {
        ...
        // userland module.
        this._methods = (0, _autoimplementmethods.autoImplementMethods)(userland);
        ...
        if (this.nextConfigOutput === 'export') {
            if (this._dynamic === 'force-dynamic') {
                throw Object.defineProperty(new Error(`export const dynamic = "force-dynamic" on page "${this.definition.pathname}" cannot be used with "output: export". See more info here: https://nextjs.org/docs/advanced-features/static-html-export`), "__NEXT_ERROR_CODE", {
                    value: "E278",
                    enumerable: false,
                    configurable: true
                });
            } else if (!(0, _isstaticgenenabled.isStaticGenEnabled)(userland) && userland['GET']) {
                throw Object.defineProperty(new Error(`export const dynamic = "force-static"/export const revalidate not configured on route "${this.definition.pathname}" with "output: export". See more info here: https://nextjs.org/docs/advanced-features/static-html-export`), "__NEXT_ERROR_CODE", {
                    value: "E301",
                    enumerable: false,
                    configurable: true
                });
        ...
        if (process.env.NODE_ENV === 'development') {
            ...
            // response.
            if (!_http.HTTP_METHODS.some((method)=>method in userland)) {
   ...
   */ resolveHandlerFromUserland(method, userland) {
        if (!(0, _http.isHTTPMethod)(method)) return ()=>new Response(null, {
                status: 400
            });
    ...
    async handle(req, context) {
        ...
        const rawLiveUserland = this._getUserland == null ? void 0 : this._getUserland.call(this);
        const liveUserland = rawLiveUserland instanceof Promise ? undefined : rawLiveUserland;
        const handler = liveUserland ? this.resolveHandlerFromUserland(req.method, liveUserland) : this.resolveHandler(req.method);
        ...
        const response = await this.actionAsyncStorage.run(actionStore, ()=>this.workUnitAsyncStorage.run(requestStore, ()=>this.workAsyncStorage.run(workStore, async ()=>{
                    ...
                    const hasNonStatic = liveUserland ? hasNonStaticMethods(liveUserland) : this._hasNonStaticMethods;
                    if (hasNonStatic) {
                    ...
                    const dynamic = (liveUserland == null ? void 0 : liveUserland.dynamic) ?? this._dynamic;
