    ...
    // Deno are not affected by this check.
    {
        ...
        if (strategy === 'workerThreads' || isForced) {
            ...
            if (!isBun && !isDeno) {
                ...
                if (!_semver.default.satisfies(nodeVersion, WORKER_THREADS_SAFE_RANGE, {
                    includePrerelease: true
                })) {
                    if (isForced) {
                        _log.warn(`\`experimental.turbopackPluginRuntimeStrategy = ` + `'forceWorkerThreads'\` has been enabled, but you're using ` + `Node.js ${nodeVersion}, which has known memory safety bugs ` + `with worker threads used from the Node-API. You may ` + `experience crashes, segmentation faults, or other ` + `instability. Upgrade to Node.js ${WORKER_THREADS_SAFE_RANGE}.`);
                    } else {
                        _log.warn(`\`experimental.turbopackPluginRuntimeStrategy = ` + `'workerThreads'\` is set but has been ` + `ignored because you're using Node.js ${nodeVersion}, which ` + `has memory safety bugs in worker threads. Falling back to ` + `'childProcesses'. Upgrade to Node.js ` + `${WORKER_THREADS_SAFE_RANGE}.`);
...
async function loadConfig(phase, dir, { customConfig, rawConfig, silent = true, reportExperimentalFeatures, reactProductionProfiling, debugPrerender, bundler } = {}) {
    ...
    if (path == null ? void 0 : path.length) {
        ...
        if (((_userConfig_experimental = userConfig.experimental) == null ? void 0 : _userConfig_experimental.supportsImmutableAssets) && bundler !== undefined && bundler !== _bundler.Bundler.Turbopack) {
        ...
        if (((_userConfig_experimental1 = userConfig.experimental) == null ? void 0 : _userConfig_experimental1.lightningCssFeatures) && !((_userConfig_experimental2 = userConfig.experimental) == null ? void 0 : _userConfig_experimental2.useLightningcss) && bundler !== _bundler.Bundler.Turbopack) {
        ...
        if (phase !== _constants.PHASE_PRODUCTION_SERVER && ((_userConfig_experimental3 = userConfig.experimental) == null ? void 0 : _userConfig_experimental3.useLightningcss)) {
