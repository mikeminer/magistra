 ...
 */ async function callGenerateStaticParams(generateStaticParams, parentParams, rootParamKeys, implicitTags) {
    const rootParams = {};
    ...
    return _workunitasyncstorageexternal.workUnitAsyncStorage.run(workUnitStore, generateStaticParams, {
        params: parentParams
    });
}
async function generateRouteStaticParams(segments, store, isRoutePPREnabled, rootParamKeys) {
    ...
    while(queue.length > 0){
        ...
        if (params.length > 0) {
            ...
            for (const parentParams of params){
                const result = await callGenerateStaticParams(current.generateStaticParams, parentParams, rootParamKeys, implicitTags);
                if (result.length > 0) {
            ...
            const result = await callGenerateStaticParams(current.generateStaticParams, {}, rootParamKeys, implicitTags);
...
async function buildAppStaticPaths({ dir, page, route, distDir, cacheComponents, authInterrupts, useCacheTimeout, staticPageGenerationTimeout, segments, isrFlushToDisk, cacheHandler, cacheLifeProfiles, requestHeaders, cacheHandlers, cacheMaxMemorySize, fetchCacheKeyPrefix, nextConfigOutput, ComponentMod, isRoutePPREnabled = false, partialFallbacksEnabled = false, buildId, rootParamKeys }) {
    ...
    const routeParams = await _workasyncstorageexternal.workAsyncStorage.run(store, generateRouteStaticParams, segments, store, isRoutePPREnabled, rootParamKeys);
