 ...
 */ function getTreeViewDisplayPath(pagePath) {
    ...
    return (0, _getmetadataroute.fillStaticMetadataSegment)(segment, lastSegment);
}
...
function getTreeViewSymbol(item, pageInfo) {
    if (item === '/_app' || item === '/_app.server') {
        return ' ';
    }
    if ((0, _isedgeruntime.isEdgeRuntime)(pageInfo == null ? void 0 : pageInfo.runtime)) {
        return 'ƒ';
    }
    if (pageInfo == null ? void 0 : pageInfo.isRoutePPREnabled) {
        ...
        if (!(pageInfo == null ? void 0 : pageInfo.hasPostponed)) {
            return '○';
        }
        return '◐';
    }
    if (pageInfo == null ? void 0 : pageInfo.isStatic) {
        return '○';
    }
    if (pageInfo == null ? void 0 : pageInfo.isSSG) {
        return '●';
    }
    return 'ƒ';
}
...
async function printTreeView(lists, pageInfos, { pagesDir, pageExtensions, middlewareManifest, functionsConfigManifest, useStaticPages404, hasGSPAndRevalidateZero }) {
    ...
    const printFileTree = async ({ list, routerType })=>{
        ...
        filteredPages.forEach((item, i, arr)=>{
            var _pageInfo_ssgPageDurations, _pageInfo_ssgPageRoutes, _pageInfo_ssgPageRoutes1;
            ...
            const hasChildRoutes = Boolean(pageInfo == null ? void 0 : (_pageInfo_ssgPageRoutes = pageInfo.ssgPageRoutes) == null ? void 0 : _pageInfo_ssgPageRoutes.length);
            ...
            messages.push([
                `${border} ${hasChildRoutes ? ' ' : symbol} ${displayPath}${totalDuration > MIN_DURATION ? ` (${getPrettyDuration(totalDuration)})` : ''}`,
                showRevalidate && (pageInfo == null ? void 0 : pageInfo.initialCacheControl) ? (0, _format.formatRevalidate)(pageInfo.initialCacheControl) : '',
                showExpire && (pageInfo == null ? void 0 : pageInfo.initialCacheControl) ? (0, _format.formatExpire)(pageInfo.initialCacheControl) : ''
            ]);
            if (pageInfo == null ? void 0 : (_pageInfo_ssgPageRoutes1 = pageInfo.ssgPageRoutes) == null ? void 0 : _pageInfo_ssgPageRoutes1.length) {
                ...
                routes.forEach(({ route, duration, avgDuration }, index, { length })=>{
                    ...
                    messages.push([
                        `${contSymbol} ${innerSymbol} ${routeSymbol} ${route}${duration > MIN_DURATION ? ` (${getPrettyDuration(duration)})` : ''}${avgDuration && avgDuration > MIN_DURATION ? ` (avg ${getPrettyDuration(avgDuration)})` : ''}`,
...
async function isPageStatic({ dir, page, distDir, configFileName, httpAgentOptions, locales, defaultLocale, parentId, pageRuntime, edgeInfo, pageType, cacheComponents, authInterrupts, useCacheTimeout, staticPageGenerationTimeout, originalAppPath, isrFlushToDisk, cacheMaxMemorySize, nextConfigOutput, cacheHandler, cacheHandlers, cacheLifeProfiles, pprConfig, partialFallbacksEnabled, buildId, clientAssetToken, sriEnabled }) {
    ...
    return isPageStaticSpan.traceAsyncFn(async ()=>{
        ...
        if (pageType === 'app') {
            ...
            const route = (0, _app1.parseNormalizedAppRoute)(page);
