 ...
 */ function buildRDCCacheByPage(results, finalPhaseExportPaths) {
    ...
    const getKnownParamsKey = (normalizedPage, path, fallbackParamNames)=>{
        let params;
        try {
            params = (0, _getparams.getParams)(normalizedPage, path);
        } catch  {
            return null;
        }
...
async function exportAppImpl(dir, options, span, staticWorker) {
    ...
    if (telemetry) {
        telemetry.record((0, _events.eventCliSession)(nextConfig, {
            ...
            isCustomServer: null,
            turboFlag: options.bundler === _bundler.Bundler.Turbopack,
            pagesDir: null,
            appDir: null
        }));
    }
  ...
  Read more: https://nextjs.org/docs/messages/export-image-api`), "__NEXT_ERROR_CODE", {
    ...
    const renderOpts = {
        ...
        experimental: {
            ...
            maxPostponedStateSizeBytes: (0, _configshared.parseMaxPostponedStateSize)(nextConfig.experimental.maxPostponedStateSize)
        },
    ...
    const exportPagesInBatches = async (worker, exportPaths, renderResumeDataCachesByPage)=>{
        ...
        return (await Promise.all(batches.map(async (batch)=>worker.exportPages({
                ...
                clientAssetToken: nextConfig.experimental.supportsImmutableAssets ? '' : nextConfig.deploymentId,
                exportPaths: batch,
