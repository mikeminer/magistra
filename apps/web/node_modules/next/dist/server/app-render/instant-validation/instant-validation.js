...
async function collectStagedSegmentData(ComponentMod, renderFlightStream, fullPageChunks, fullPageDebugChunks, startTime, hasRuntimePrefetch, clientReferenceManifest, createDebugChannel) {
...
async function createCombinedPayloadStream(ComponentMod, renderFlightStream, payload, extraChunksAbortController, renderSignal, clientReferenceManifest, startTime, isDebugChannelEnabled, createDebugChannel) {
...
async function createCombinedPayloadAtDepth(initialRSCPayload, cache, initialLoaderTree, getDynamicParamFromSegment, query, depth, groupDepth, releaseSignal, boundaryState, clientReferenceManifest, stageEndTimes, useRuntimeStageForPartialSegments) {
   ...
   */ function wrapSlotsWithMarkers(slots, results) {
        ...
        for (const key of keys){
            ...
            const markerName = `${_boundaryconstants.INSTANT_SLOT_MARKER_PREFIX}${markerIndex - 1}${_boundaryconstants.INSTANT_SLOT_MARKER_SUFFIX}`;
            ...
            slots[key] = [
                ...
                /*#__PURE__*/ (0, _jsxruntime.jsx)(_boundary.SlotMarker, {
                    name: markerName,
                    children: node
                }, "sm"),
                parallelRoutesData,
    ...
    async function buildSharedTreeSeedData(loaderTree, parentPath, key, urlDepthConsumed, groupDepthConsumed) {
        ...
        if (isBoundary) {
            ...
            for(const parallelRouteKey in parallelRoutes){
                const result = await buildNewTreeSeedData(parallelRoutes[parallelRouteKey], path, parallelRouteKey, false, 0 /* segmentDepth */ );
    ...
    async function buildNewTreeSeedData(lt, parentPath, key, isInsideRuntimePrefetch, segmentDepth) {
        ...
        if (layoutOrPageMod !== undefined) {
            ...
            prefetchConfig = layoutOrPageMod.unstable_prefetch ?? null;
        ...
        for(const parallelRouteKey in parallelRoutes){
            ...
            const result = await buildNewTreeSeedData(parallelRoutes[parallelRouteKey], path, parallelRouteKey, childIsInsideRuntimePrefetch, childSegmentDepth);
            slotResults.set(parallelRouteKey, result);
            slots[parallelRouteKey] = result.seedData;
            if (result.requiresInstantUI) {
