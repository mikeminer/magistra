 ...
 */ async function generateStagedDynamicFlightRenderResultWeb(req, ctx, requestStore) {
    ...
    const flightReadableStream = await (0, _apprenderrenderutils.runInSequentialTasks)(()=>{
        ...
        if (requestStore.varyParamsAccumulator) {
            void (0, _varyparams.finishAccumulatingVaryParams)(requestStore.varyParamsAccumulator);
        }
    }, ()=>{
...
async function stagedRenderWithoutCachesInDevWeb(ctx, requestStore, getPayload, options) {
    ...
    return await (0, _apprenderrenderutils.runInSequentialTasks)(()=>{
        stageController.advanceStage(_stagedrendering.RenderStage.Static);
        return _workunitasyncstorageexternal.workUnitAsyncStorage.run(requestStore, _streamops.renderToWebFlightStream, ctx.componentMod, rscPayload, clientModules, {
            ...options,
            environmentName
        });
    }, ()=>{
    ...
    !isBypassingCachesInDev(initialRequestStore)) {
        ...
        if (shouldValidate) {
            ...
            if (returnedDebugChannel) {
                const [t1, t2] = (0, _streamops.teeStream)(returnedDebugChannel.clientSide.readable);
        ...
        if (process.env.__NEXT_USE_NODE_STREAMS) {
            ...
            debugChannel = setReactDebugChannel && (0, _debugchannelserver.createWebDebugChannel)();
...
async function getRSCPayload(tree, ctx, options) {
    ...
    const initialTree = await (0, _createflightrouterstatefromloadertree.createFlightRouterStateFromLoaderTree)(tree, hints, prefetchInliningEnabled, ctx.renderOpts.cacheComponents, workStore.isStaticGeneration, ctx.renderOpts.isBuildTimePrerendering ?? false, getDynamicParamFromSegment, query);
    const serveStreamingMetadata = !!ctx.renderOpts.serveStreamingMetadata;
...
async function getErrorRSCPayload(tree, ctx, ssrError, errorType) {
    ...
    const initialTree = await (0, _createflightrouterstatefromloadertree.createFlightRouterStateFromLoaderTree)(tree, errorHints, errorPrefetchInliningEnabled, ctx.renderOpts.cacheComponents, workStore.isStaticGeneration, ctx.renderOpts.isBuildTimePrerendering ?? false, getDynamicParamFromSegment, query);
    let err = undefined;
    if (ssrError) {
    ...
    return (0, _tracer.getTracer)().withSpan(renderSpan, async ()=>{
        ...
        try {
            ...
            if (process.env.__NEXT_USE_NODE_STREAMS) {
                ...
                const { stream: htmlStream, allReady } = await _workunitasyncstorageexternal.workUnitAsyncStorage.run(requestStore, _streamops.renderToNodeFizzStream, appElement, fizzOptions);
                // End the render span only after React completed rendering (including anything inside Suspense boundaries)
                allReady.finally(()=>{
                    if (renderSpan.isRecording()) renderSpan.end();
                });
...
async function renderWithRestartOnCacheMissInDevWeb(ctx, initialRequestStore, createRequestStore, getPayload, onError) {
    ...
    requestStore.cacheSignal = cacheSignal;
    let debugChannel = setReactDebugChannel && (0, _debugchannelserver.createWebDebugChannel)();
    const { clientModules } = (0, _manifestssingleton.getClientReferenceManifest)();
    ...
    const initialStreamResult = await (0, _apprenderrenderutils.runInSequentialTasks)(()=>{
        ...
        const streamPair = (0, _streamops.teeStream)(_workunitasyncstorageexternal.workUnitAsyncStorage.run(requestStore, _streamops.renderToWebFlightStream, ComponentMod, initialRscPayload, clientModules, {
            onError,
            environmentName,
            startTime,
            filterStackFrame,
    ...
    const finalStreamResult = await (0, _apprenderrenderutils.runInSequentialTasks)(()=>{
        finalStageController.advanceStage(_stagedrendering.RenderStage.EarlyStatic);
        startTime = performance.now() + performance.timeOrigin;
        const streamPair = (0, _streamops.teeStream)(_workunitasyncstorageexternal.workUnitAsyncStorage.run(requestStore, _streamops.renderToWebFlightStream, ComponentMod, finalRscPayload, clientModules, {
            onError,
            environmentName,
            startTime,
            filterStackFrame,
...
async function renderWithRestartOnCacheMissInDevNode(ctx, initialRequestStore, createRequestStore, getPayload, onError) {
    ...
    requestStore.asyncApiPromises = createAsyncApiPromises(initialStageController, requestStore.cookies, requestStore.mutableCookies, requestStore.headers);
    requestStore.cacheSignal = cacheSignal;
    let debugChannel = setReactDebugChannel && (0, _debugchannelserver.createNodeDebugChannel)();
    const { clientModules } = (0, _manifestssingleton.getClientReferenceManifest)();
    // Note: The stage controller starts out in the `Before` stage,
    // where sync IO does not cause aborts, so it's okay if it happens before render.
    const initialRscPayload = await getPayload(requestStore);
    const initialStreamResult = await (0, _apprenderrenderutils.runInSequentialTasks)(()=>{
        initialStageController.advanceStage(_stagedrendering.RenderStage.EarlyStatic);
        startTime = performance.now() + performance.timeOrigin;
        const sourceStream = _workunitasyncstorageexternal.workUnitAsyncStorage.run(requestStore, _streamops.renderToNodeFlightStream, ComponentMod, initialRscPayload, clientModules, {
            onError,
            environmentName,
            startTime,
            filterStackFrame,
    ...
    const finalStreamResult = await (0, _apprenderrenderutils.runInSequentialTasks)(()=>{
        finalStageController.advanceStage(_stagedrendering.RenderStage.EarlyStatic);
        startTime = performance.now() + performance.timeOrigin;
        const finalSourceStream = _workunitasyncstorageexternal.workUnitAsyncStorage.run(requestStore, _streamops.renderToNodeFlightStream, ComponentMod, finalRscPayload, clientModules, {
            onError,
            environmentName,
            startTime,
            filterStackFrame,
        ...
        const finalReplayable = new _apprenderprerenderutils.ReplayableNodeStream(finalSourceStream);
        return {
...
async function prerenderToStream(req, res, ctx, metadata, tree, fallbackRouteParams) {
    ...
    try {
       ...
       */ // The prerender controller represents the lifetime of the prerender. It
            ...
            if (serverIsDynamic) {
                ...
                if (hasFallbackRouteParams) {
                    ...
                    const emptyReactServerResult = await (0, _apprenderprerenderutils.createReactServerPrerenderResultFromRender)((0, _streamops.renderToWebFlightStream)(ComponentMod, [], clientModules, {
                        filterStackFrame,
                        onError: serverComponentsErrorHandler
                    }));
                    finalStream = await (0, _streamops.continueStaticFallbackPrerender)(htmlStream, {
                        inlinedDataStream: (0, _streamops.createWebInlinedDataStream)(emptyReactServerResult.consumeAsStream(), nonce, formState),
                        getServerInsertedHTML,
                        getServerInsertedMetadata,
                        deploymentId: ctx.sharedContext.deploymentId
                    });
                    ...
                    finalStream = await (0, _streamops.continueStaticPrerender)(htmlStream, {
                        inlinedDataStream: (0, _streamops.createWebInlinedDataStream)(reactServerResult.consumeAsStream(), nonce, formState),
                        getServerInsertedHTML,
                        getServerInsertedMetadata,
                        deploymentId: ctx.sharedContext.deploymentId
                    });
            ...
            reactServerResult = reactServerPrerenderResult = await (0, _apprenderprerenderutils.createReactServerPrerenderResultFromRender)(_workunitasyncstorageexternal.workUnitAsyncStorage.run(reactServerPrerenderStore, _streamops.renderToWebFlightStream, ComponentMod, RSCPayload, clientModules, {
                filterStackFrame,
                onError: serverComponentsErrorHandler
            }));
       ...
       */ // First we check if we have any dynamic holes in our HTML prerender
            ...
            let reactServerResult;
            reactServerResult = reactServerPrerenderResult = await (0, _apprenderprerenderutils.createReactServerPrerenderResultFromRender)(_workunitasyncstorageexternal.workUnitAsyncStorage.run(prerenderLegacyStore, _streamops.renderToWebFlightStream, ComponentMod, RSCPayload, clientModules, {
                filterStackFrame,
                onError: serverComponentsErrorHandler
            }));
            const { stream: htmlStream } = await _workunitasyncstorageexternal.workUnitAsyncStorage.run(prerenderLegacyStore, _streamops.renderToWebFizzStream, // eslint-disable-next-line @next/internal/no-ambiguous-jsx
            /*#__PURE__*/ (0, _jsxruntime.jsx)(App, {
                reactServerStream: reactServerResult.asUnclosingStream(),
                reactDebugStream: undefined,
                debugEndTime: undefined,
        ...
        const errorServerStreamRaw = _workunitasyncstorageexternal.workUnitAsyncStorage.run(prerenderLegacyStore, _streamops.renderToWebFlightStream, ComponentMod, errorRSCPayload, clientModules, {
            filterStackFrame,
            onError: serverComponentsErrorHandler
        });
        ...
        const errorFlightResultPromise = prerenderHTTPError ? (()=>{
            ...
            // gets its own copy.
            const [appStream, flightStream] = (0, _streamops.teeStream)(errorServerStreamRaw);
            errorServerStream = appStream;
            return (0, _apprenderprerenderutils.createReactServerPrerenderResultFromRender)(flightStream);
        })() : null;
        try {
            const { stream: errorHtmlStream } = await _workunitasyncstorageexternal.workUnitAsyncStorage.run(prerenderLegacyStore, _streamops.renderToWebFizzStream, // eslint-disable-next-line @next/internal/no-ambiguous-jsx
            ...
            return {
                ...
                stream: await (0, _streamops.continueFizzStream)(errorHtmlStream, {
                    inlinedDataStream: (0, _streamops.createWebInlinedDataStream)(flightStream, nonce, formState),
                    isStaticGeneration: true,
                    getServerInsertedHTML: (0, _makegetserverinsertedhtml.makeGetServerInsertedHTML)({
                        polyfills,
                        renderServerInsertedHTML,
