...
import { PAGES_MANIFEST, BUILD_ID_FILE, MIDDLEWARE_MANIFEST, PREFETCH_HINTS, PRERENDER_MANIFEST, ROUTES_MANIFEST, CLIENT_PUBLIC_FILES_PATH, APP_PATHS_MANIFEST, SERVER_DIRECTORY, NEXT_FONT_MANIFEST, UNDERSCORE_NOT_FOUND_ROUTE_ENTRY, FUNCTIONS_CONFIG_MANIFEST } from '../shared/lib/constants';
...
export default class NextNodeServer extends BaseServer {
    constructor(options){
        ...
        super(options), this.cleanupListeners = new AsyncCallbackSet(), this.handleNextImageRequest = async (req, res, parsedUrl)=>{
            ...
            // the `else` branch is needed for tree-shaking
                ...
                if (!this.imageCacheHandler && this.nextConfig.images.customCacheHandler) {
                    ...
                    if (cacheHandler) {
                        const CacheHandler = interopDefault(await dynamicImportEsmDefault(formatDynamicImportPath(this.distDir, cacheHandler)));
            ...
            const existingServerContext = routerServerGlobal[RouterServerContextSymbol][relativeProjectDir];
            if (!existingServerContext) {
                routerServerGlobal[RouterServerContextSymbol][relativeProjectDir] = {
                    render404: this.render404.bind(this)
                };
            }
            routerServerGlobal[RouterServerContextSymbol][relativeProjectDir].nextConfig = this.nextConfig;
            routerServerGlobal[RouterServerContextSymbol][relativeProjectDir].isWrappedByNextServer = true;
            try {
     ...
     */ if (this.renderOpts.optimizeCss) {
        ...
        this.middlewareManifestPath = join(/* turbopackIgnore: true */ this.serverDistDir, MIDDLEWARE_MANIFEST);
    ...
    async loadInstrumentationModule() {
        if (!this.serverOptions.dev) {
            try {
                this.instrumentation = await getInstrumentationModule(this.dir, this.nextConfig.distDir);
            } catch (err) {
    ...
    loadEnvConfig({ dev, forceReload }) {
        loadEnvConfig(this.dir, dev, Log, forceReload, forceReload ? (envFilePath)=>{
    ...
    getPagesManifest() {
        return loadManifest(join(/* turbopackIgnore: true */ this.serverDistDir, PAGES_MANIFEST));
    }
    getAppPathsManifest() {
        if (!this.enabledDirectories.app) return undefined;
        return loadManifest(join(/* turbopackIgnore: true */ this.serverDistDir, APP_PATHS_MANIFEST));
    }
    ...
    getBuildId() {
        const buildIdFile = join(/* turbopackIgnore: true */ this.distDir, BUILD_ID_FILE);
        try {
            return fs.readFileSync(/* turbopackIgnore: true */ buildIdFile, 'utf8').trim();
        } catch (err) {
            if (err.code === 'ENOENT') {
    ...
    async runApi(req, res, query, match) {
        ...
        addRequestMeta(req.originalRequest, 'relativeProjectDir', relative(process.cwd(), this.dir));
        ...
        // the `else` branch is needed for tree-shaking
            ...
            if (this.enabledDirectories.app && renderOpts.isAppPath) {
                ...
                null, renderOpts, this.getServerComponentsHmrCache(), {
                    ...
                    clientAssetToken: this.nextConfig.experimental.supportsImmutableAssets ? '' : this.deploymentId
                });
            } else {
                ...
                return lazyRenderPagesPage(req.originalRequest, res.originalResponse, pathname, query, renderOpts, {
                    buildId: this.buildId,
                    deploymentId: this.deploymentId,
                    clientAssetToken: this.nextConfig.experimental.supportsImmutableAssets ? undefined : this.deploymentId,
    ...
    getNextFontManifest() {
        return loadManifest(join(/* turbopackIgnore: true */ this.distDir, 'server', NEXT_FONT_MANIFEST + '.json'));
    }
   ...
   */ getEdgeFunctionInfo(params) {
        ...
        return {
            name: pageInfo.name,
            paths: pageInfo.files.map((file)=>join(/* turbopackIgnore: true */ this.distDir, file)),
            wasm: (pageInfo.wasm ?? []).map((binding)=>({
                    ...binding,
                    filePath: join(/* turbopackIgnore: true */ this.distDir, binding.filePath)
                })),
    ...
    async loadNodeMiddleware() {
        if (!process.env.NEXT_MINIMAL) {
            try {
                ...
                const functionsConfig = this.dev ? {} : require(join(/* turbopackIgnore: true */ this.distDir, 'server', FUNCTIONS_CONFIG_MANIFEST));
                if (this.dev || (functionsConfig == null ? void 0 : (_functionsConfig_functions = functionsConfig.functions) == null ? void 0 : _functionsConfig_functions['/_middleware'])) {
                    ...
                    return require(join(/* turbopackIgnore: true */ this.distDir, 'server', 'middleware.js'));
                }
            } catch (err) {
   ...
   */ async runMiddleware(params) {
    ...
    getPrerenderManifest() {
        ...
        this._cachedPreviewManifest = loadManifest(join(/* turbopackIgnore: true */ this.distDir, PRERENDER_MANIFEST));
        return this._cachedPreviewManifest;
    }
    getPrefetchHints() {
        ...
        this._cachedPrefetchHints = loadManifest(join(/* turbopackIgnore: true */ this.distDir, SERVER_DIRECTORY, PREFETCH_HINTS), true, undefined, false, true // handleMissing: don't crash if the file doesn't exist
        ) ?? {};
        return this._cachedPrefetchHints;
    }
    getRoutesManifest() {
        return getTracer().trace(NextNodeServerSpan.getRoutesManifest, ()=>loadManifest(join(/* turbopackIgnore: true */ this.distDir, ROUTES_MANIFEST)));
    }
    ...
    get serverDistDir() {
        ...
        const serverDistDir = join(/* turbopackIgnore: true */ this.distDir, SERVER_DIRECTORY);
