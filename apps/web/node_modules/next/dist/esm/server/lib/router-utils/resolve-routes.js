...
import { NEXT_REWRITTEN_PATH_HEADER, NEXT_REWRITTEN_QUERY_HEADER, RSC_HEADER } from '../../../client/components/app-router-headers';
...
export function getResolveRoutes(fsChecker, config, opts, renderServer, renderServerOpts, ensureMiddleware) {
    ...
    if (process.env.__NEXT_TEST_MODE && process.env.IS_TURBOPACK_TEST) {
        try {
            clientHashes = JSON.parse(require('fs').readFileSync(path.join(opts.dir, config.distDir, 'immutable-static-hashes.json'), 'utf8'));
        } catch  {}
    }
    let routes = null;
            ...
            if (params) {
                ...
                if (route.name === 'middleware_next_data' && parsedUrl.pathname) {
                    ...
                    if ((_fsChecker_getMiddlewareMatchers = fsChecker.getMiddlewareMatchers()) == null ? void 0 : _fsChecker_getMiddlewareMatchers.length) {
                        ...
                        const isNextDataPath = pathHasPrefix(normalized, '/_next/data') && normalized.endsWith('.json');
                        ...
                        if (updated) {
                            if (hadBasePath) {
                                normalized = normalized === '/' ? config.basePath : path.posix.join(config.basePath, normalized);
                            }
                    ...
                    if (output && !(config.i18n && (initialLocaleResult == null ? void 0 : initialLocaleResult.detectedLocale) && pathHasPrefix(pathname, '/api'))) {
                        if (config.useFileSystemPublicRoutes || didRewrite || output.type !== 'appFile' && output.type !== 'pageFile') {
                            ...
                            if (process.env.__NEXT_TEST_MODE && process.env.IS_TURBOPACK_TEST && output.type === 'nextStaticFolder' && config.deploymentId) {
                                let isImmutableFile = config.experimental.supportsImmutableAssets && clientHashes[`static${decodeURI(output.itemPath)}`];
                                const expectedToken = isImmutableFile ? undefined : config.deploymentId;
