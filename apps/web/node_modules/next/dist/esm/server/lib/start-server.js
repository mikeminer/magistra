...
import { ensureAgentRulesForDev, getEnvInfo, logExperimentalInfo, logStartInfo } from './app-info-log';
...
export async function startServer(serverOptions) {
    ...
    process.title = `next-server (v${"16.3.0-canary.6"})`;
    let handlersReady = ()=>{};
    let handlersError = ()=>{};
                    ...
                    if (initResult.agentRules !== false) {
                        ...
                        if (result) {
                            ...
                            if (generated.length > 0) {
                                Log.event(`Generated ${generated.join(' and ')} for AI agents. Set \`agentRules: false\` in next.config to disable.`);
                            }
                        }
                    }
                }
    ...
    if (isDev) {
        ...
        while(true){
            ...
            if (!nextAncestor.startsWith(dir + path.sep) || nextAncestor === prevAncestor) {
                break;
            }
