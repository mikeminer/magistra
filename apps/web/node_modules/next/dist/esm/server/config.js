...
import { CONFIG_FILES, PHASE_DEVELOPMENT_SERVER, PHASE_EXPORT, PHASE_PRODUCTION_BUILD, PHASE_PRODUCTION_SERVER } from '../shared/lib/constants';
 ...
 */ function assignDefaultsAndValidate(dir, userConfig, silent, phase) {
    ...
    if (process.env.__NEXT_TEST_MODE && process.env.IS_TURBOPACK_TEST && result.deploymentId && process.env.__NEXT_SUPPORTS_IMMUTABLE_ASSETS) {
        result.experimental.supportsImmutableAssets = true;
    }
    if (process.env.NEXT_HASH_SALT) {
        result.experimental.outputHashSalt = (result.experimental.outputHashSalt ?? '') + process.env.NEXT_HASH_SALT;
    }
    ...
    if (result.cacheHandlers) {
        ...
        for (const key of handlerKeys){
            if (key === 'private') {
                ...
                const resolvedHandlerPath = handlerPath && resolveCacheHandlerPathToFilesystem(handlerPath);
    ...
    // Deno are not affected by this check.
    {
        ...
        if (strategy === 'workerThreads' || isForced) {
            ...
            if (!isBun && !isDeno) {
                ...
                if (!semver.satisfies(nodeVersion, WORKER_THREADS_SAFE_RANGE, {
                    includePrerelease: true
                })) {
                    if (isForced) {
                        Log.warn(`\`experimental.turbopackPluginRuntimeStrategy = ` + `'forceWorkerThreads'\` has been enabled, but you're using ` + `Node.js ${nodeVersion}, which has known memory safety bugs ` + `with worker threads used from the Node-API. You may ` + `experience crashes, segmentation faults, or other ` + `instability. Upgrade to Node.js ${WORKER_THREADS_SAFE_RANGE}.`);
                    } else {
                        Log.warn(`\`experimental.turbopackPluginRuntimeStrategy = ` + `'workerThreads'\` is set but has been ` + `ignored because you're using Node.js ${nodeVersion}, which ` + `has memory safety bugs in worker threads. Falling back to ` + `'childProcesses'. Upgrade to Node.js ` + `${WORKER_THREADS_SAFE_RANGE}.`);
...
async function applyModifyConfig(config, phase, silent) {
    ...
    if (config.adapterPath) {
        const adapterMod = interopDefault(await import(pathToFileURL(require.resolve(config.adapterPath)).href));
...
export default async function loadConfig(phase, dir, { customConfig, rawConfig, silent = true, reportExperimentalFeatures, reactProductionProfiling, debugPrerender, bundler } = {}) {
    // Generate cache key based on parameters that affect config output
    ...
    if (path == null ? void 0 : path.length) {
        var _userConfig_experimental, _userConfig_experimental1, _userConfig_experimental2, _userConfig_experimental3;
        ...
        if (((_userConfig_experimental = userConfig.experimental) == null ? void 0 : _userConfig_experimental.supportsImmutableAssets) && bundler !== undefined && bundler !== Bundler.Turbopack) {
        ...
        if (((_userConfig_experimental1 = userConfig.experimental) == null ? void 0 : _userConfig_experimental1.lightningCssFeatures) && !((_userConfig_experimental2 = userConfig.experimental) == null ? void 0 : _userConfig_experimental2.useLightningcss) && bundler !== Bundler.Turbopack) {
        ...
        if (phase !== PHASE_PRODUCTION_SERVER && ((_userConfig_experimental3 = userConfig.experimental) == null ? void 0 : _userConfig_experimental3.useLightningcss)) {
            var _css, _this, _userConfig_experimental4;
            const { loadBindings } = require('../build/swc');
            const isLightningSupported = (_this = await loadBindings((_userConfig_experimental4 = userConfig.experimental) == null ? void 0 : _userConfig_experimental4.useWasmBinary)) == null ? void 0 : (_css = _this.css) == null ? void 0 : _css.lightning;
...
function enforceExperimentalFeatures(config, options) {
    ...
    if (process.env.__NEXT_EXPERIMENTAL_APP_NEW_SCROLL_HANDLER === 'true' && // We do respect an explicit value in the user config.
    (config.experimental.appNewScrollHandler === undefined || isDefaultConfig && !config.experimental.appNewScrollHandler)) {
        ...
        if (configuredExperimentalFeatures) {
            addConfiguredExperimentalFeature(configuredExperimentalFeatures, 'appNewScrollHandler', true, 'enabled by `__NEXT_EXPERIMENTAL_APP_NEW_SCROLL_HANDLER`');
        }
    }
    // Enable node streams via env var (for CI testing).
    if (process.env.__NEXT_USE_NODE_STREAMS === 'true' && (config.experimental.useNodeStreams === undefined || isDefaultConfig && !config.experimental.useNodeStreams)) {
