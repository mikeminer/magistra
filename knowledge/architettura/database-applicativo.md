...
È **distinto concettualmente** dall'[indice normativo](/architettura/indice-normativo.md), che contiene il corpus pubblico e i suoi embedding. Possono però condividere la stessa istanza.

## Caratteristiche

- Relazionale, basato sul **dialetto Postgres** (coerente con l'uso di `pgvector` per l'indice).
- Istanza **PGlite** embedded nel bundle dell'[app desktop](/architettura/deployment.md): Postgres in WASM, in-process, senza installazione né servizi esterni.
